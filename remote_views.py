...
if not openai_api_key_env:
    print("WARNING: OPENAI_API_KEY environment variable not set. OpenAI features will not work.")
    openai.api_key = None
else:
...
if not stripe_secret_key_env:
    print("WARNING: STRIPE_SECRET_KEY environment variable not set. Payment features might not work correctly.")
if not runpod_api_key_env:
    print("WARNING: RUNPOD_API_KEY environment variable not set. RunPod features might not work correctly.")

payment_manager = PaymentManager(stripe_secret_key=stripe_secret_key_env)
if hasattr(db, 'session'):
    payment_manager.set_db_session(db.session)
else:
    print("ERROR: Database session (db.session) not available or db not fully initialized when PaymentManager is configured.")
...
class CasiView(BaseView):
    ...
    def tool(self):
        ...
        available_backends = ["openai", "anthropic", "google"] # "groq" is not ready
        ...
        if request.method == 'POST':
            ...
            if action == 'save_keys':
                ...
                gen_output, _ = casi.generator(
                    ...
                    model=getattr(casi.config, f"{context['selected_gen_backend']}_model"),
                ...
                crit_output, _ = casi.critic(
                    ...
                    model=getattr(casi.config, f"{context['selected_crit_backend']}_model"),
                ...
                gen_api_key = session.get('openai_api_key') if context['selected_gen_backend'] == 'openai' else \
                              session.get('anthropic_api_key') if context['selected_gen_backend'] == 'anthropic' else None
                
                crit_api_key = session.get('openai_api_key') if context['selected_crit_backend'] == 'openai' else \
                               session.get('anthropic_api_key') if context['selected_crit_backend'] == 'anthropic' else None
                ...
                results = casi.run_automatic_cycle(
                    ...
                    gen_model=getattr(casi.config, f"{context['selected_gen_backend']}_model"),
                    gen_prompt=context['generator_prompt'],
                    gen_api_key=gen_api_key,
                    crit_backend=context['selected_crit_backend'],
                    crit_model=getattr(casi.config, f"{context['selected_crit_backend']}_model"),
